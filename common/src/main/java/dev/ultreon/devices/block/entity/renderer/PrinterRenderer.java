...
public record PrinterRenderer(
    ...
    private static void renderPaper(@NotNull PoseStack pose, @NotNull MultiBufferSource bufferSource,
                                    int packedLight, BlockState state, PaperModel paperModel) {
    ...
    private static void renderPrint(PrinterBlockEntity blockEntity, @NotNull PoseStack pose, BlockState
            state, @NotNull MultiBufferSource bufferSource, int packedLight, int packedOverlay, PaperModel paperModel) {
    ...
    private static void drawBuffer(@NotNull PoseStack pose, @NotNull MultiBufferSource bufferSource,
                                   int packedLight, PaperModel paperModel) {
        VertexConsumer buffer = bufferSource.getBuffer(paperModel.renderType(PaperModel.TEXTURE));
        paperModel.renderToBuffer(pose, buffer, packedLight, OverlayTexture.NO_OVERLAY, 0xffffffff);
    }

    private static void renderDisplay(PrinterBlockEntity blockEntity, @NotNull PoseStack
            pose, @NotNull MultiBufferSource bufferSource, BlockState state) {
