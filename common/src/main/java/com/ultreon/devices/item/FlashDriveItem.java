...
public class FlashDriveItem extends Item implements Colored, SubItems, IDeviceType {
    ...
    public FlashDriveItem(Properties properties, DyeColor color) {
        super(properties.arch$tab(OmnixerioDevicesMod.TAB_DEVICE).rarity(Rarity.UNCOMMON).stacksTo(1));
        this.color = color;
    }
    ...
    public NonNullList<ResourceLocation> getModels() {
        ...
        for (DyeColor color : DyeColor.values())
            modelLocations.add(OmnixerioDevicesMod.id(Objects.requireNonNull(RegistrarManager.getId(this, Registries.ITEM)).getPath().substring(5) + "/" + color.getName()));
