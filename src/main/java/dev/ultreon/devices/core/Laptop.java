 ...
 * Laptop GUI class.
 *
 * @author MrCrayfish, QboiDev
 */
public class Laptop extends Screen implements System {
    public static final int ID = 1;
    public static final int ICON_SIZE = 14;
    private static final Identifier LAPTOP_FONT = OmnixerioDevicesCommon.id("laptop");
    private static Font font;
    private static final Identifier LAPTOP_GUI = Identifier.fromNamespaceAndPath(OmnixerioDevicesCommon.MOD_ID, "laptop");
    ...
    public static List<Application> getApplicationsOutside() {
        return APPLICATIONS;
    }
    ...
    public Laptop(ComputerBlockEntity laptop, boolean worldLess) {
        ...
        if (currentWallpaper.isBuiltIn()) {
            wallpaper.setImageSprite(WALLPAPERS.get(currentWallpaper.location));
        } else {
    ...
    public CompoundTag getModSystemTag(ModContainer mod) {
        return getModSystemTag(mod.getMetadata().getId());
    }
...
//                    }
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        try {
            ...
            try {
                ...
                // Ignored because it's not always possible to disable scissor.'
            }
            bsod(e);
        }
    }
    ...
    public static void drawLines(GuiGraphicsExtractor graphics, Font font, String text, int x, int y, int width, int color) {
        ...
        font.getSplitter().splitLines(FormattedText.of(text.replace("\r\n", "\n").replace("\r", "\n")), width, Style.EMPTY).forEach(b -> lines.add(b.getString()));
        ...
        textScale = 1f / Minecraft.getInstance().getWindow().getGuiScale();
        ...
        var lineNr = 0;
        for (String s : lines) {
            graphics.text(font, s.replace("\t", "    "), 0, lineNr * font.lineHeight, color);
            lineNr++;
        }
        graphics.pose().popMatrix();
    }
    ...
    public void renderLaptop(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, float partialTicks) {
        ...
        Image.CACHE.forEach((_, cachedImage) -> cachedImage.delete());
            ...
            for (int i = windows.size() - 1; i >= 0; i--) {
                ...
                if (window != null) {
                    ...
                    try {
                        ...
                        graphics.disableScissor();
                        OmnixerioDevicesCommon.LOGGER.error("An error has occurred while rendering a window.", e);
                        ...
                        if (window.content instanceof Application app) {
                            ...
                            openApplication(ApplicationManager.getApplication(OmnixerioDevicesCommon.id("diagnostics")), intent);
    ...
    private void bsod(Throwable e) {
        ...
        OmnixerioDevicesCommon.LOGGER.error("An error has occurred while rendering the laptop.", e);
    }
...
//        this.minecraft.dispatchKeypresses();
    ...
    private Application openApplication(Application app, CompoundTag intent) {
        ...
        try {
            ...
            e.printStackTrace();
            AppInfo info = ApplicationManager.getApplication(OmnixerioDevicesCommon.id("diagnostics"));
    ...
    public Pair<Application, Boolean> openApplication(AppInfo info, File file) {
        ...
        try {
            ...
            e.printStackTrace();
            AppInfo info1 = ApplicationManager.getApplication(OmnixerioDevicesCommon.id("diagnostics"));
...
//        }
    ...
    private boolean isValidApplication(AppInfo info) {
        if (OmnixerioDevicesCommon.hasAllowedApplications()) {
            return OmnixerioDevicesCommon.getAllowedApplications().contains(info);
        }
        return true;
    }
