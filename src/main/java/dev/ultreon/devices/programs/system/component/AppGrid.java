...
public class AppGrid extends Component {
    ...
    private Layout generateAppTile(AppEntry entry, int left, int top) {
        ...
        if (entry instanceof LocalEntry localEntry) {
         ...
         //   dev.ultreon.component.app.api.devices.Image image = new dev.ultreon.component.app.api.devices.Image(iconOffset, padding, 14 * 3, 14 * 3, localEntry.info().getIconU(), localEntry.info().getIconV(), 14, 14, 224, 224, Laptop.ICON_TEXTURES);
            ...
            dev.ultreon.devices.api.app.component.Image image = new dev.ultreon.devices.api.app.component.Image(iconOffset, padding, 14 * 3, 14 * 3, AppStore.CERTIFICATES_BASE_URL + "/assets/" + resource.getNamespace() + "/" + resource.getPath() + "/icon.png");
        ...
        if (store.certifiedApps.contains(entry)) {
            dev.ultreon.devices.api.app.component.Image certifiedIcon = new dev.ultreon.devices.api.app.component.Image(15, 38, Icons.VERIFIED);
        ...
        if (entry instanceof LocalEntry) {
            ...
            if (Laptop.getSystem().getInstalledApplications().contains(info)) {
                dev.ultreon.devices.api.app.component.Image installedIcon = new dev.ultreon.devices.api.app.component.Image(itemWidth - 10 - 15, 38, Icons.CHECK);
    ...
    private void reloadIcons(Layout layout) {
        layout.components.forEach(component -> {
            ...
            } else if (component instanceof dev.ultreon.devices.api.app.component.Image) {
                ((dev.ultreon.devices.api.app.component.Image) component).reload();
            }
        });
    }
}
