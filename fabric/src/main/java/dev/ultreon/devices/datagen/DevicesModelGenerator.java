...
public class DevicesModelGenerator extends FabricModelProvider {
    ...
    public void generateBlockStateModels(BlockModelGenerators blockStateModelGenerator) {
        ...
        ModBlocks.CLOCKS.getMap().forEach((dye, block) -> {
            blockStateModelGenerator.modelOutput.accept(ModelLocationUtils.getModelLocation(block.get()), () -> new Gson().fromJson(String.format(clockPain(), dye.getName()), JsonElement.class));
            //blockStateModelGenerator.modelOutput.accept(ModelLocationUtils.getModelLocation(block.get()), () -> new Gson().fromJson(String.format(laptopPain(), dye.getName()), JsonElement.class));
            blockStateModelGenerator.blockStateOutput.accept(new BlockStateGenerator() {
                @NotNull
                @Override
                ...
                @Override
                public JsonElement get() {
                    return new Gson().fromJson(String.format(clockStatePain(), dye.getName()), JsonElement.class);
                }
            });
    ...
    private String clockPain() {
        try {
            FileInputStream d = new FileInputStream(dataOutput.getModContainer().findPath("clock/clockpain.txt").orElseThrow().toFile());
            return IOUtils.toString(d, StandardCharsets.UTF_8);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    private String clockStatePain() {
        try {
            FileInputStream d = new FileInputStream(dataOutput.getModContainer().findPath("clock/clockstatepain.txt").orElseThrow().toFile());
            return IOUtils.toString(d, StandardCharsets.UTF_8);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
